...
export function ThreeSimulationViewport({ cockpit }: { cockpit: CockpitData }) {
  ...
  useEffect(() => {
    ...
    scene.fog = new THREE.Fog(0x08100f, 13, 30);

    const camera = new THREE.PerspectiveCamera(45, 1, 0.1, 80);
    camera.position.set(8, 9, 12);
    camera.lookAt(0, 0, 0);

    const ambient = new THREE.AmbientLight(0xb6dbc6, 0.35);
    scene.add(ambient);

    const hemi = new THREE.HemisphereLight(0x94c7ff, 0x203415, 0.65);
    scene.add(hemi);
    ...
    function createObstacles() {
      ...
      [[-5.2, -2.5], [4.9, -0.6], [6.2, 2.1], [-2.1, 3.6]].forEach(([x, z], index) => {
      ...
      [[-6.8, 3.9], [7.1, -3.6], [2.6, 4.6]].forEach(([x, z]) => {
